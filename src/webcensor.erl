...
add_url(From, Table, URL) when is_list(URL) ->
    add_url(From, Table, list_to_binary(URL));
add_url(_From, Table, URL) when is_binary(URL) ->
    case URL of
	<<"http://", _Rest/binary>> ->
...
terminate(Table) ->
    ...
    file:write_file(?BLOCK_LIST_FILE, list_to_binary(AllURLs)),
    ok.
...
react_to(_Server, Client, Data = <<"GET ", Rest/binary>>) ->
...
react_to(_Server, Client, Data = <<"POST ", Rest/binary>>) ->
...
connect(Host, Port) when is_list(Host) ->
